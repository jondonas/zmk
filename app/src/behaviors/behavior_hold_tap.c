...
struct behavior_hold_tap_config {
    ...
    int32_t include_prior_idle_key_positions_len;
    int32_t *include_prior_idle_key_positions;
    int32_t cancel_prior_idle_key_positions_len;
    int32_t *cancel_prior_idle_key_positions;
    enum flavor flavor;
...
struct behavior_hold_tap_data {
    int64_t last_prior_idle_key_timestamp;
...
// keycode listener to update per-instance prior-idle timestamps with the correct
...
static bool is_include_prior_idle_key_position(const struct behavior_hold_tap_config *config,
                                              int32_t position) {
    for (int i = 0; i < config->include_prior_idle_key_positions_len; i++) {
        if (config->include_prior_idle_key_positions[i] == position) {
            return true;
        }
    }
    return false;
...
static bool is_cancel_prior_idle_key_position(const struct behavior_hold_tap_config *config,
                                             int32_t position) {
    for (int i = 0; i < config->cancel_prior_idle_key_positions_len; i++) {
        if (config->cancel_prior_idle_key_positions[i] == position) {
            return true;
        }
    }
    return false;
...
// Registry of all hold-tap device instances, populated during init, so the
// keycode listener can update each instance's per-instance prior-idle timestamp.
#define HT_MAX_DEVICES DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT)
static const struct device *ht_devices[HT_MAX_DEVICES];
static int ht_num_devices = 0;

// For instances with a cancel list, track a per-instance "last prior idle key"
// timestamp. A key in the cancel list actively resets the timestamp to INT32_MIN
// (cancelling the idle window), so a hold-tap pressed right after it is free to
// hold even if a non-cancel key was pressed very recently. Any other key arms the
// window as usual.
static void update_prior_idle_key_timestamps(int32_t position, int64_t timestamp) {
    for (int i = 0; i < ht_num_devices; i++) {
        ...
        if (cfg->cancel_prior_idle_key_positions_len > 0) {
            if (is_cancel_prior_idle_key_position(cfg, position)) {
...
static bool is_quick_tap(struct active_hold_tap *hold_tap) {
    // With a cancel list, the prior-idle window is gated by the per-instance timestamp
    // (which cancel keys reset); otherwise it uses the global last_tapped timestamp.
    int64_t prior_idle_timestamp = hold_tap->config->cancel_prior_idle_key_positions_len > 0
                                       ? hold_tap->data->last_prior_idle_key_timestamp
                                       : last_tapped.timestamp;
    if ((prior_idle_timestamp + hold_tap->config->require_prior_idle_ms) > hold_tap->timestamp) {
        return true;
    }
...
static struct active_hold_tap *store_hold_tap(struct zmk_behavior_binding_event *event,
    ...
    for (int i = 0; i < ZMK_BHV_HOLD_TAP_MAX_HELD; i++) {
        ...
        active_hold_taps[i].quick_tap_deferred = false;
...
static int on_hold_tap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    struct active_hold_tap *hold_tap =
        store_hold_tap(&event, binding->param1, binding->param2, cfg, data);
    ...
    if (is_quick_tap(hold_tap)) {
        if (hold_tap->config->include_prior_idle_key_positions_len > 0) {
...
static int on_hold_tap_binding_released(struct zmk_behavior_binding *binding,
    ...
    if (hold_tap->quick_tap_deferred) {
        hold_tap->quick_tap_deferred = false;
...
static int position_state_changed_listener(const zmk_event_t *eh) {
    ...
    // If the combo key is in include_prior_idle_key_positions, fire the deferred quick-tap now.
    // If the combo key is NOT in the list, skip quick-tap entirely and let normal flavor logic
    // handle the decision (as if require-prior-idle-ms was not set for that interaction).
    if (ev->state && undecided_hold_tap->quick_tap_deferred) {
        undecided_hold_tap->quick_tap_deferred = false;
        if (is_include_prior_idle_key_position(undecided_hold_tap->config, ev->position)) {
...
static int behavior_hold_tap_init(const struct device *dev) {
    ...
    if (ht_num_devices < HT_MAX_DEVICES) {
        ht_devices[ht_num_devices++] = dev;
    }
...
#define KP_INST(n)                                                                                 \
    COND_CODE_1(DT_INST_NODE_HAS_PROP(n, hold_trigger_key_positions),                              \
                ...
                ())                                                                                \
    COND_CODE_1(DT_INST_NODE_HAS_PROP(n, include_prior_idle_key_positions),                         \
                (static const int32_t include_prior_idle_key_positions_##n[] =                      \
                     DT_INST_PROP(n, include_prior_idle_key_positions);),                           \
                ())                                                                                \
    COND_CODE_1(DT_INST_NODE_HAS_PROP(n, cancel_prior_idle_key_positions),                          \
                (static const int32_t cancel_prior_idle_key_positions_##n[] =                       \
                     DT_INST_PROP(n, cancel_prior_idle_key_positions);),                            \
                ())                                                                                \
    static const struct behavior_hold_tap_config behavior_hold_tap_config_##n = {                  \
        ...
        .include_prior_idle_key_positions =                                                        \
            COND_CODE_1(DT_INST_NODE_HAS_PROP(n, include_prior_idle_key_positions),                \
                        (include_prior_idle_key_positions_##n), (NULL)),                           \
        .include_prior_idle_key_positions_len =                                                    \
            DT_INST_PROP_LEN(n, include_prior_idle_key_positions),                                 \
        .cancel_prior_idle_key_positions =                                                         \
            COND_CODE_1(DT_INST_NODE_HAS_PROP(n, cancel_prior_idle_key_positions),                 \
                        (cancel_prior_idle_key_positions_##n), (NULL)),                            \
        .cancel_prior_idle_key_positions_len =                                                     \
            DT_INST_PROP_LEN(n, cancel_prior_idle_key_positions),                                  \
